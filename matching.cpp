...
bool checkL(POKEMON** content, int x1, int y1, int x2, int y2)
{
	...
	if (checkI(content, x1, y1, x2, y1) && checkI(content, x2, y2, x2, y1) && content[x1][y2].data == 0)
		return true;
	if (checkI(content, x1, y1, x1, y2) && checkI(content, x2, y2, x1, y2) && content[x2][y1].data == 0)
		return true;
